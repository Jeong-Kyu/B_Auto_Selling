...
class MyWindow(QMainWindow):
    ...
    def request_opt10081(self):
        now = datetime.datetime.now()
        today = now.strftime("%Y%m%d")
        self.SetInputValue("종목코드", "054630")
        ...
        if real_type == "장시작시간":
            ...
            if 장운영구분 == '3':
                if self.previous_day_hold:
                    self.previous_day_hold = False
                    # 매도 (시장가)
                    self.SendOrder("매도", "8001", self.account, 2, "054630", self.previous_day_quantity, 0, "03", "")
            elif 장운영구분 == '4':
            ...
            if self.hold is None and self.target is not None and self.amount is not None and 현재가 > self.target:
                ...
                self.SendOrder("매수", "8000", self.account, 1, "054630", quantity, 0, "03", "")
    ...
    def subscribe_stock_conclusion(self, screen_no):
        self.SetRealReg(screen_no, "054630", "20", 0)
