...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        self.call_on = 0
        self.put_on = 0
        self.target_call_price = 0       # 콜행사가
        self.target_call_price_p = 0   # 콜가격
        self.target_put_price = 0        # 풋행사가
        self.target_put_price_p = 0  # 풋가격
        ###############################################################
        self.sjmd = -0.2                   # 손절
        self.ijmd = 0.2                    # 익절
        ...
        self.portfolio_stock_dict ={f'201T3{self.target_call_price}':{"현재가":0},f'301T3{self.target_put_price}':{"현재가":0}}
        ...
        print('[콜행사가:%s, 콜가격:%s], [풋행사가:%s, 풋가격:%s]' % (self.target_call_price,self.target_call_price_p,self.target_put_price,self.target_put_price_p))
        print(self.dict)
        ...
        if sRQName == '종목가격':
            ...
            if self.ic in self.c_present_price:
                ...
                if self.z_c > abs(1-abs(float(price.strip()))):
                    ...
                    self.target_call_price_p = float(self.c_present_price[self.ic]['현재가'])
                    print(self.target_call_price,self.target_call_price_p)
                ...
                if self.z_p > abs(1-abs(float(price.strip()))):
                    ...
                    self.target_put_price_p = float(self.p_present_price[self.ic]['현재가'])
                    print(self.target_put_price,self.target_put_price_p)
    ...
    def call_price_check(self):
        ...
        k=self.dynamicCall('CommRqData(String, String, int, String)','콜행사가',"opt50004","0", self.screen_my_info)
