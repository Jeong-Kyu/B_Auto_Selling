        ...
        if sRealType == '장시작시간':
            ...
            if l == 0:
                ...
                if int(self.now_cp['call']) == 0:
                    self.now_cp['call'] = 1
                    gr = self.dynamicCall('SendOrderFO(QString, QString, QString, QString, int, int, QString, int, int, QString)',['콜매수', '0101', self.account_num, f'201T4{int(self.target_call_price)}', 1, 2, '4', self.msml, 0, ""])
                    if gr == 0:
                        print('콜매수')
                    # self.now_cp.update({'call':(int(self.now_cp['call'])+1)})
                    # print(self.now_cp)
            elif l <= self.target_price+0.05 and self.hjg > self.target_price+0.05:
                ...
                if int(self.now_cp['put']) == 0:
                    self.now_cp['put'] = 1
                    gr = self.dynamicCall('SendOrderFO(QString, QString, QString, QString, int, int, QString, int, int, QString)',['풋매수', '0101', self.account_num, f'301T4{int(self.target_put_price)}', 1, 2, '4', self.msml, 0, ""])
            ...
            if sCode in self.jango_dict.keys():
                ...
                if self.foline[sCode]['1'] != 0:
                    ...
                    # if self.foline[sCode]['1'] > realp:
                        ...
                        print(f'기준수정 : {self.target_price}')
